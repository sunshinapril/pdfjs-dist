...
class PDFViewer {
  ...
  setDocument(pdfDocument) {
    ...
    Promise.all([firstPagePromise, permissionsPromise]).then(([firstPdfPage, permissions]) => {
      ...
      // 添加水印功能
      eventBus._on("pagerendered", ({ source }) => {
        ...
        // 水印文本
        const watermarktext = AppOptions.get("watermarktext") || "";
        console.log("页面渲染完成，水印文本:", watermarktext);
        ...
        const lineHeight = 48 // Line height
        
        // Define watermark positions (two positions per page)
        const positions = [
          { x: width * 0.7, y: height * 0.3 }, // Left middle area
          { x: width * 0.3, y: height * 0.7 }  // Right middle area
        ]
        
        // Draw watermarks at specified positions
...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    if(!AppOptions.get("watermarktext")) {
      AppOptions.set("watermarktext", params.get('watermarktext') || '');
    }
...
PDFPrintServiceFactory.initGlobals(PDFViewerApplication);
{
  ...
  const WHITE_LIST = new Set(['https://mini-app.cfc108.com:8445', 'https://mini-images.linkqh.com'])
  var validateFileURL = function (file) {
    if (!file) {
      return;
    }
    const isWhiteList = [...WHITE_LIST].filter(item => file.includes(item));
    if(isWhiteList && isWhiteList.length) {
      return;
    }
    console.log(file,'file', HOSTED_VIEWER_ORIGINS)
